...
if [ -z ${3} ]; then
    echo "ERROR - You must specify an Ethernet interface or | seperated list"
    error_level=1;
fi
...
if [ $error_level -ne 0 ]; then
    echo "Usage: ${0} 192.168.122.25 24 enp1s0 start|stop"
    exit $error_level
fi
...
case "${4}" in
	...
	if [ $? -ne 0 ]; then
	    ...
	    nmcli con mod ${ETHINT} +ipv4.address ${1}/${2}
	    error_level=$?
	    if [ $error_level -ne 0 ]; then 
		echo "ERROR - Command failed: nmcli con mod ${ETHINT} +ipv4.address ${1}/${2}"
	    fi
